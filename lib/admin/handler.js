...
import {
  ...
  isSameOrigin, ownerLinkCookie, readOwnerLink, readSession, sessionCookie, verifyPassword,
} from "./auth.js";
...
import { createDiskStorage, createStorage, imageKeyFor, sniffImageType, storageConfig } from "./storage.js";
import {
...
function storageFor(deps) {
  ...
  if (config.configured) return createStorage(config);
  // Development: a folder stands in for the bucket, so photos can be tried
  // locally with nothing installed. Only scripts/serve.mjs sets this, and only
  // off Vercel, so a hosted deployment can never write to a disk.
  const dir = String(deps.env.LOCAL_UPLOADS_DIR ?? "").trim();
  if (dir && !deps.env.VERCEL) return createDiskStorage(dir);
  return null;
}
export async function uploadStatus(env, storage) {
  const config = storageConfig(env);
  const hosted = Boolean(env.VERCEL);
  if (storage?.disk) return { enabled: true, maxBytes: MAX_UPLOAD_BYTES, local: true };
  if (!config.configured || !storage) {
    return {
      ...
      detail: hosted
        ...
        : "Le reste fonctionne. En local, les photos sont enregistrées dans .local/uploads sans rien installer : arrêtez le serveur et relancez npm run dev.",
    };
  }
  if (config.local) {
...
async function login(request, url, config, deps) {
  ...
  if (!isSameOrigin(request)) {
    return html(403, renderLogin({ next, error: "Ce formulaire de connexion a expiré. Rechargez la page puis réessayez." }), env);
  }
  const address = clientAddress(request);
  const wait = deps.throttle.retryAfter(address, deps.now());
  if (wait) {
    const minutes = Math.max(1, Math.ceil(wait / 60));
    return html(429, renderLogin({ next, error: `Trop de tentatives infructueuses. Attendez ${minutes} minute${minutes === 1 ? "" : "s"}, puis réessayez.` }), env, { "Retry-After": String(wait) });
  }
  // Only the password is asked for. ADMIN_USERNAME is not a credential: it is
  // the name the change history records.
  if (!(await verifyPassword(String(form.get("password") ?? ""), config.passwordHash))) {
    deps.throttle.fail(address, deps.now());
    return html(401, renderLogin({ next, error: "Le mot de passe est incorrect. Vérifiez-le puis réessayez." }), env);
  }
...
async function upload(request, url, session, deps) {
  ...
  // A photo arrives as two parts: "display" is the crop the menu shows, and
  // "original" is the whole photo it was cut from, sent whenever a new file
  // was chosen. Reframing an existing photo sends only "display" and the
  // stored original stays, so a crop is never cut from another crop.
  const declared = (request.headers.get("content-type") ?? "").split(";")[0].trim().toLowerCase();
  const parts = new Map();
  if (declared === "multipart/form-data") {
    const length = Number(request.headers.get("content-length"));
    if (Number.isFinite(length) && length > MAX_UPLOAD_BYTES) throw new AdminError(413, "Cette photo est trop volumineuse. Choisissez-en une de moins de 4 Mo.");
    let form;
    try {
      form = await request.formData();
    } catch {
      throw new AdminError(400, "L’envoi de la photo est incomplet. Réessayez.");
    }
    for (const name of ["display", "original"]) {
      const part = form.get(name);
      if (part && typeof part.arrayBuffer === "function") parts.set(name, new Uint8Array(await part.arrayBuffer()));
    }
    if (!parts.has("display")) throw new AdminError(400, "La photo recadrée est absente de l’envoi. Réessayez.");
  } else {
    // A single image, as older clients and simple tools send it.
    if (!IMAGE_TYPES[declared]) throw new AdminError(415, "Choisissez une photo JPEG, PNG ou WebP.");
    try {
      parts.set("display", await readBody(request, MAX_UPLOAD_BYTES));
    } catch (error) {
      if (error.status === 413) throw new AdminError(413, "Cette photo est trop volumineuse. Choisissez-en une de moins de 4 Mo.");
      throw error;
    }
  }
  const total = [...parts.values()].reduce((sum, bytes) => sum + bytes.length, 0);
  if (total > MAX_UPLOAD_BYTES) throw new AdminError(413, "Cette photo est trop volumineuse. Choisissez-en une de moins de 4 Mo.");

  // Each part's own bytes decide its type, exactly as a single upload does.
  const typed = new Map();
  for (const [name, bytes] of parts) {
    ...
    typed.set(name, { bytes, type });
  }

  const db = openDb(deps);
  const target = await resolveImageTarget(db, venue, spec);
  const named = (part) => imageKeyFor({ venue, target: target.target, slot: target.slot, id: target.id, slug: target.slug }, part.type);
  const key = named(typed.get("display"));
  const originalKey = typed.has("original") ? named(typed.get("original")) : undefined;
  const written = [];
  try {
    await storage.putObject(key, typed.get("display").bytes, typed.get("display").type);
    written.push(key);
    if (originalKey) {
      await storage.putObject(originalKey, typed.get("original").bytes, typed.get("original").type);
      written.push(originalKey);
    }
  } catch (error) {
    console.error(`[admin] upload to storage failed: ${error.message}`);
    for (const done of written) await storage.deleteObject(done).catch(() => {});
  ...
  let result;
  try {
    result = await setImageKey(db, { venue, actor: session.username, ...spec, key, originalKey });
  } catch (error) {
    for (const done of written) await storage.deleteObject(done).catch(() => {});
    throw error;
  }
...
// Hands the photo now in use back to the console, from this origin, so the
// browser can draw it on a canvas and crop it again. Without this the canvas
// would be tainted by the storage domain and could export nothing. Only a
// signed-in session reaches it, and only the key this venue actually uses.
const CONTENT_TYPES = { webp: "image/webp", jpg: "image/jpeg", png: "image/png" };
async function imageSource(url, deps) {
  const { env } = deps;
  const venue = assertVenue(url.searchParams.get("venue"));
  ...
  const storage = storageFor(deps);
  if (!storage?.getObject) throw new AdminError(503, "La photo d’origine ne peut pas être relue ici.");
  // The whole photo, so every crop is cut from the original rather than from
  // the previous crop. Photos saved before originals were kept have none; for
  // those the crop itself is handed back, which is all there is.
  const resolved = await resolveImageTarget(openDb(deps), venue, { target, id, slot });
  const key = resolved.currentOriginalKey ?? resolved.currentKey;
  if (!key) throw new AdminError(404, "Cette photo n’existe plus. Rechargez la page.");
  let object;
  try {
    object = await storage.getObject(key);
  } catch (error) {
    console.error(`[admin] could not read ${key} back: ${error.message}`);
    object = null;
  }
  if (!object) throw new AdminError(404, "Cette photo est introuvable dans le stockage. Choisissez-en une nouvelle.");
  return respond(200, object.body, env, CONTENT_TYPES[key.split(".").pop()] ?? "application/octet-stream");
}
...
async function api(request, url, op, config, deps) {
  ...
  try {
    if (request.method !== "POST") {
      ...
      if (op === "image.source") return await imageSource(url, deps);
