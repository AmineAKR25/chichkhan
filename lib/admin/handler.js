...
import {
  LoginThrottle, adminConfig, clearSessionCookie, clientAddress, createSessionToken, isSameOrigin,
  readSession, safeEqual, sessionCookie, verifyPassword,
} from "./auth.js";
...
// Everything under /admin and /api/admin. Pages arrive as ?page=… through
// the rewrites in vercel.json, API calls as /api/admin?op=…. Every route
// except sign-in checks the signed session cookie on the server, and every
// state-changing request must come from this site's own admin pages.
...
async function loginPage(request, url, config, deps) {
  const next = safeNext(url.searchParams.get("next"));
  if (readSession(request, config, deps.now())) return redirect(next || "/admin", deps.env);
  const notice = url.searchParams.has("signedout") ? "Vous êtes déconnecté·e." : url.searchParams.has("expired") ? "Votre session a expiré. Connectez-vous de nouveau." : "";
  return html(200, renderLogin({ next, notice }), deps.env);
}
...
async function login(request, url, config, deps) {
  ...
  return redirect(next || "/admin", env, { "Set-Cookie": sessionCookie(request, createSessionToken(config, deps.now())) });
}

function logout(request, env) {
  if (!isSameOrigin(request)) return redirect("/admin", env);
  return redirect("/admin/login?signedout=1", env, { "Set-Cookie": clearSessionCookie(request) });
}

function requireSessionForPage(request, url, config, deps, path) {
  const session = readSession(request, config, deps.now());
  if (session) return { session };
  const next = safeNext(path + url.search.replace(/[?&]page=[^&]*/g, "").replace(/^&/, "?"));
  return { response: redirect(`/admin/login${next && next !== "/admin" ? `?next=${encodeURIComponent(next)}` : ""}`, deps.env) };
}

async function dashboard(request, url, config, deps) {
  const { session, response } = requireSessionForPage(request, url, config, deps, "/admin");
...
async function editor(request, url, venue, config, deps) {
  const { session, response } = requireSessionForPage(request, url, config, deps, `/admin/${venue}`);
  if (response) return response;
  try {
...
async function api(request, url, op, config, deps) {
  const { env } = deps;
  const session = readSession(request, config, deps.now());
  if (!session) return apiError(401, "Votre session a expiré. Connectez-vous de nouveau pour continuer.", env, { code: "signed-out" });
...
export async function handleAdminRequest(request, options = {}) {
  ...
  try {
    ...
    if (page === "login") return isPost ? await login(request, url, config, deps) : await loginPage(request, url, config, deps);
    if (page === "logout") return isPost ? logout(request, env) : redirect("/admin", env);
    if (isPost) return respond(405, "Method not allowed", env, "text/plain; charset=utf-8", { Allow: "GET, HEAD" });
    ...
    return html(404, renderMessagePage({ title: "Introuvable", heading: "Page introuvable.", message: "Cette page d’administration n’existe pas.", status: "info" }), env);
  } catch (error) {
    console.error(`[admin] ${error?.stack ?? error}`);
    return op
      ? apiError(500, "Une erreur est survenue sur le serveur. Aucune modification n’a été effectuée. Réessayez dans un instant.", env)
      : html(500, renderMessagePage({ title: "Erreur", heading: "Une erreur est survenue.", message: "La page ne peut pas être affichée. Réessayez dans un instant." }), env);
  }
}
