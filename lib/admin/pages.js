...
function archFigure({ url, alt = "", color, className = "" }) {
  return `<figure class="arch ${className}${url ? " has-image" : ""}" data-image style="--photo-color:${color}">${url ? `<img src="${escape(url)}" alt="${escape(alt)}" loading="lazy" decoding="async">` : ""}</figure>`;
}
...
export function renderEditor({ session, venue, state }) {
  ...
  return adminDocument({
    ...
    // The status bar takes the band's own colour, so the phone frames each
    // venue the way its page does.
    themeColor: isRestaurant ? "#e5d8bf" : "#10322f",
    themeColorDark: isRestaurant ? "#16110d" : "#0b1612",
