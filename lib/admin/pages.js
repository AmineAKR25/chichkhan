...
export function renderLogin({ username = "", next = "", error = "", notice = "" } = {}) {
  const action = `/admin/login${next ? `?next=${encodeURIComponent(next)}` : ""}`;
  const invalid = error ? ' aria-invalid="true" aria-describedby="login-error"' : "";
  return adminDocument({
    title: "Connexion",
    bodyClass: "auth-body",
    body: `<main id="main" class="auth-page"><div class="auth-card">${brandLogo(64)}<p class="eyebrow">Chichkhan</p><h1>Administration des menus</h1><p class="auth-lead">Connectez-vous pour modifier les menus du Café et du Restaurant.</p>${notice ? `<p class="notice notice-success" role="status">${icon("check")}<span>${escape(notice)}</span></p>` : ""}${error ? `<div class="form-alert" id="login-error" role="alert">${icon("alert")}<div><strong>Connexion impossible.</strong><p>${escape(error)}</p></div></div>` : ""}<form method="post" action="${escape(action)}" class="auth-form"><input type="hidden" name="next" value="${escape(next)}"><div class="field"><label for="username">Identifiant</label><input id="username" name="username" autocomplete="username" autocapitalize="none" spellcheck="false" required maxlength="200" value="${escape(username)}"${invalid}${username ? "" : " autofocus"}></div><div class="field"><label for="password">Mot de passe</label><input id="password" name="password" type="password" autocomplete="current-password" required maxlength="1024"${invalid}${username ? " autofocus" : ""}><label class="check check-inline"><input type="checkbox" data-show-password="password"><span>Afficher le mot de passe</span></label></div><button class="btn btn-primary btn-large btn-block" type="submit" data-pending-label="Connexion…">Se connecter</button></form><p class="auth-foot">Cet espace est privé. Les clients ne le voient jamais et les menus publics ne créent aucun lien vers lui.</p></div></main>`,
  });
}
...
export function renderMessagePage({ title, heading, message, session, status = "error" }) {
  return adminDocument({
    ...
    body: `${session ? topBar(session) : ""}<main id="main" class="message-page"><div class="message-card">${icon(status === "error" ? "alert" : "info", 28)}<h1>${escape(heading)}</h1><p>${escape(message)}</p><div class="button-row"><a class="btn btn-primary" href="${session ? "/admin" : "/admin/login"}">${session ? "Retour à l’administration" : "Aller à la connexion"}</a><a class="btn btn-secondary" href="">Réessayer</a></div></div></main>`,
  });
}
