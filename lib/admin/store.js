...
export async function loadVenueState(q, venue) {
  ...
  const venueRow = (await q.query(
    "select slug, name, title, subtitle, eyebrow, hero_image_key, hero_image_alt, hero_focus_y, logo_image_key from venues where slug = $1::venue", [venue])).rows[0];
  ...
  return {
    venue: {
      ...
      heroFocusY: Number(venueRow.hero_focus_y ?? 50),
...
export async function loadVenueSummary(q, venue) {
  ...
  const row = (await q.query(`
    select v.slug, v.name, v.title, v.subtitle, v.hero_image_key, v.hero_focus_y,
  ...
  return {
    ...
    heroFocusY: Number(row.hero_focus_y ?? 50),
...
async function getCategory(tx, venue, id) {
  const row = (await tx.query(`
    select id, group_id, slug, name, note, image_key, is_visible, position from categories
...
async function getProduct(tx, venue, id) {
  const row = (await tx.query(`
    select id, category_id, name, description, price_millimes, is_house, is_visible, image_key, position from menu_items
...
async function deleteCategory(context, input) {
  ...
  let snapshot;
  let summary;
  let imageKeys = [category.image_key];
  ...
  if (mode === "empty") {
    ...
    imageKeys = [...imageKeys, ...products.map((row) => row.image_key)];
...
async function deleteProduct(context, input) {
  ...
  const undo = await recordDeletion(context, { recordType: "product", recordId: product.id, recordName: product.name, snapshot: { product: row }, imageKeys: [product.image_key] });
...
const VENUE_FIELDS = {
  ...
  heroFocusY: { column: "hero_focus_y", label: "partie affichée de la photo principale" },
};
const focusWord = (value) => ({ 0: "haute", 50: "centrale", 100: "basse" })[value] ?? "choisie";
...
async function updateVenue(context, input) {
  ...
  const changed = fields.filter((field) => (field === "heroFocusY" ? Number(current.hero_focus_y) : current[VENUE_FIELDS[field].column]) !== data[field]);
  ...
  const summary = changed.length === 1 && changed[0] === "heroFocusY"
    ? `La photo principale de ${page} affiche maintenant sa partie ${focusWord(data.heroFocusY)}.`
    : `L’en-tête de ${page} a été mis à jour (${changed.map((field) => VENUE_FIELDS[field].label).join(", ")}).`;
  await context.audit({
...
export async function resolveImageTarget(q, venue, { target, id, slot }) {
  ...
  if (target === "venue") {
    ...
    const row = (await q.query(`select name, ${column} as image_key from venues where slug = $1::venue`, [venue])).rows[0];
    if (!row) throw new AdminError(404, "Ce menu n’est pas encore dans la base de données.");
    return { target, slot, id: null, column, currentKey: row.image_key, label: slot === "hero" ? `la photo principale du ${VENUE_LABELS[venue]}` : `le logo du ${VENUE_LABELS[venue]}`, recordName: row.name, recordType: "venue" };
  }
  if (target === "category") {
    const row = await getCategory(q, venue, id);
    return { target, id: row.id, slug: row.slug, column: "image_key", currentKey: row.image_key, label: quote(row.name), recordName: row.name, recordType: "category" };
  }
  if (target === "product") {
    const row = await getProduct(q, venue, id);
    return { target, id: row.id, column: "image_key", currentKey: row.image_key, label: quote(row.name), recordName: row.name, recordType: "product" };
  }
  throw new AdminError(400, "Choisissez l’élément auquel appartient la photo.");
}

async function writeImageKey(context, spec, key) {
  const { tx, venue } = context;
  const resolved = await resolveImageTarget(tx, venue, spec);
  if (resolved.target === "venue") {
    await tx.query(`update venues set ${resolved.column} = $2 where slug = $1::venue`, [venue, key]);
  } else {
    const table = resolved.target === "category" ? "categories" : "menu_items";
    await tx.query(`update ${table} set image_key = $3 where venue = $1::venue and id = $2`, [venue, resolved.id, key]);
  }
  ...
  return {
    ...
    releasedKeys: resolved.currentKey && resolved.currentKey !== key ? [resolved.currentKey] : [],
  };
}
...
export async function setImageKey(db, { venue, actor, target, id, slot, key }) {
  return inVenue(db, venue, actor, (context) => writeImageKey(context, { target, id, slot }, key));
}
...
export async function isImageKeyReferenced(q, key) {
  const row = (await q.query(`
    select exists (select 1 from venues where hero_image_key = $1 or logo_image_key = $1)
        or exists (select 1 from categories where image_key = $1)
        or exists (select 1 from menu_items where image_key = $1)
