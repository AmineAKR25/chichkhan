...
//   ADMIN_USERNAME         optional: the name recorded in the admin history.
//                          It is not a credential and is never asked for.
...
export function adminConfig(env = process.env) {
  const missing = [];
  const problems = [];
  // Not a secret and not typed at sign-in: only the name changes are filed under.
  const username = String(env.ADMIN_USERNAME ?? "").trim() || "Propriétaire";
