...
const document = ({ title, description = "", theme = "#143a38", colorScheme = "dark", assetVersion, bodyAttributes = "", scripts = "", preconnect = "", body }) => `<!doctype html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="robots" content="noindex,nofollow"><meta name="color-scheme" content="${colorScheme}"><meta name="theme-color" content="${theme}">${description ? `<meta name="description" content="${escape(description)}">` : ""}<title>${escape(title)}</title>${preconnect ? `<link rel="preconnect" href="${escape(preconnect)}">` : ""}<link rel="icon" href="/assets/logo-96.png"><link rel="preload" href="/assets/fonts/body.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="/assets/fonts/display.woff2" as="font" type="font/woff2" crossorigin>${scripts}<link rel="stylesheet" href="/style.css?v=${assetVersion}"></head><body ${bodyAttributes}>${body}</body></html>`;
...
// The one strip that stays on screen: ways to reach the venue on the left,
// the review on the right. Icon-only, so each link carries its own name.
function contactBar(venue) {
  const tel = venue.phone ? venue.phone.replace(/[^+\d]/g, "") : "";
  const links = [
    venue.facebookUrl && [venue.facebookUrl, "facebook", "Facebook"],
    venue.instagramUrl && [venue.instagramUrl, "instagram", "Instagram"],
    tel && [`tel:${tel}`, "phone", `Appeler le ${venue.phone}`],
    venue.mapsUrl && [venue.mapsUrl, "pin", "Itinéraire sur Google Maps"],
  ].filter(Boolean);
  if (!links.length && !venue.reviewUrl) return "";
  const away = (href) => (/^https?:/.test(href) ? ' target="_blank" rel="noopener"' : "");
  const icons = links.map(([href, name, label]) => `<a href="${escape(href)}" aria-label="${escape(label)}"${away(href)}>${icon(name)}</a>`).join("");
  const review = venue.reviewUrl
    ? `<a class="contact-review" href="${escape(venue.reviewUrl)}"${away(venue.reviewUrl)}>${icon("star")}<span>Laisser un avis</span></a>`
    : "";
  return `<nav class="contact-bar" aria-label="Nous contacter"><div class="contact-bar-inner"><div class="contact-links">${icons}</div>${review}</div></nav>`;
}
...
export function renderMenuPage(data, { assetVersion = "dev", imageBaseUrl } = {}) {
  ...
  // The Restaurant's page is the cover's cream side, the Café's its green one.
  // The phone paints the status bar and the first frame from these, before any
  // stylesheet arrives; the Café keeps the defaults it has always had.
  const band = isRestaurant ? { theme: "#e5d8bf", colorScheme: "light" } : {};
  ...
  const bar = contactBar(venue);
  const hero = `<a class="skip" href="#menu-sections">Aller à la carte</a>${bar}<header class="hero" aria-labelledby="venue-title"><div class="hero-inner"><div class="hero-copy"><span class="hero-logo${logoImage ? " has-image" : ""}" data-image>${logoImage ? `<img src="${escape(logoImage)}" width="68" height="92" alt="" decoding="async" ${fallback}>` : ""}</span><div class="hero-titles"><h1 id="venue-title">${escape(venue.title)}${venue.subtitle ? ` <span>${escape(venue.subtitle)}</span>` : ""}</h1>${venue.eyebrow ? `<p class="hero-meta">${escape(venue.eyebrow)}</p>` : ""}</div></div><div class="hero-image-wrap"><figure class="hero-figure${heroImage ? " has-image" : ""}" data-image>${heroImage ? `<img class="hero-photo" src="${escape(heroImage)}" alt="${escape(venue.heroImageAlt)}" fetchpriority="high" decoding="async" ${fallback}>` : ""}</figure></div></div></header>`;

  if (!categories.length) {
    return document({
      title: venue.name, description: venue.description, assetVersion, ...band,
      bodyAttributes: `class="menu-page ${isRestaurant ? "so" : "chichkhan"}${bar ? " has-contact-bar" : ""}" data-menu="${escape(venue.slug)}"`,
