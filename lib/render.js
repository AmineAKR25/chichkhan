...
const document = ({ title, description = "", themeLight = "#16483c", themeDark = "#0c1a15", assetVersion, bodyAttributes = "", scripts = "", body }) => `<!doctype html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="robots" content="noindex,nofollow"><meta name="color-scheme" content="light dark"><meta name="theme-color" media="(prefers-color-scheme: light)" content="${themeLight}"><meta name="theme-color" media="(prefers-color-scheme: dark)" content="${themeDark}">${description ? `<meta name="description" content="${escape(description)}">` : ""}<title>${escape(title)}</title><link rel="icon" href="/assets/logo-96.png"><link rel="preload" href="/assets/fonts/body.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="/assets/fonts/display.woff2" as="font" type="font/woff2" crossorigin>${scripts}<link rel="stylesheet" href="/style.css?v=${assetVersion}"></head><body ${bodyAttributes}>${body}</body></html>`;
...
export function renderMenuPage(data, { assetVersion = "dev", imageBaseUrl } = {}) {
  ...
  const hero = `<a class="skip" href="#menu-sections">Aller à la carte</a><header class="hero" aria-labelledby="venue-title"><div class="hero-inner"><div class="hero-copy"><div class="hero-brand"><span class="hero-logo${logoImage ? " has-image" : ""}" data-image>${logoImage ? `<img src="${escape(logoImage)}" width="30" height="30" alt="" decoding="async" ${fallback}>` : ""}</span>${venue.eyebrow ? `<span>${escape(venue.eyebrow)}</span>` : ""}</div><h1 id="venue-title">${escape(venue.title)}${venue.subtitle ? ` <span>${escape(venue.subtitle)}</span>` : ""}</h1></div><div class="hero-image-wrap"><figure class="hero-figure${heroImage ? " has-image" : ""}" data-image>${heroImage ? `<img class="hero-photo" src="${escape(heroImage)}" alt="${escape(venue.heroImageAlt)}" style="object-position:50% ${Math.min(100, Math.max(0, venue.heroFocusY))}%" fetchpriority="high" decoding="async" ${fallback}>` : ""}</figure></div></div></header>`;
  ...
  <footer><div><a class="footer-brand" href="/${escape(venue.slug)}">${escape(venue.name)}</a><p>Prix en dinars tunisiens (DT)</p></div><a class="back-to-top" href="#main">Retour en haut ${icon("up")}</a></footer>
  ...
  return document({
    ...
    themeLight: isRestaurant ? "#153c4b" : "#16483c",
    themeDark: isRestaurant ? "#0b1820" : "#0c1a15",
    assetVersion,
    bodyAttributes: `class="menu-page ${isRestaurant ? "so" : "chichkhan"}" data-menu="${escape(venue.slug)}"`,
