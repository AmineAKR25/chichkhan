...
const header = (menu) => `<a class="skip" href="#main">Aller au contenu</a><header class="site-header"><a class="brand" href="/${menu.slug}" aria-label="${escape(menu.name)} — haut de page"><picture><source srcset="/assets/logo-96.webp" type="image/webp"><img src="/assets/logo-96.png" width="44" height="44" alt="" decoding="async"></picture><span>${menu.slug === "cafe" ? "Chichkhan" : "SO"}<small>${menu.slug === "cafe" ? "CAFÉ" : "RESTAURANT LOUNGE"}</small></span></a><div class="header-actions"><a class="header-menu" href="#${menu.categories[0].id}" data-category="${menu.categories[0].id}">La carte ${icon("grid")}</a></div></header>`;
