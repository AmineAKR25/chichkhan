...
- Admin sign-in comes only from `ADMIN_USERNAME`, `ADMIN_PASSWORD_HASH`, `ADMIN_SESSION_SECRET` and `ADMIN_LINK_SECRET`: no default password, no bypass. Database, storage, session and link secrets never reach the browser.
